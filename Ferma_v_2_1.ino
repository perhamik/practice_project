...
void setup() { 
  ...
  attachInterrupt(1, Check_servise_button, FALLING);
  pinMode(servis_button, INPUT);
  ...
  digitalWrite(servis_button, HIGH);
...
void Check_but(){
 ...
 void Check_servise_button(){
  if(programstate == 0){
   if(digitalRead(servis_button) == LOW){             //при нажатии первой
           servis_button_press = 1; 
           Serial.println("servis_button_press = 1;");         
       } else if(servis_button_press == 1){
            servis_button_press = 0;
            if(innerFlag == 3){
              resetFunc();
             }
             else
